...
test.describe("findMinMaxValues @12a", () => {
    test("should find min and max values in a normal array", async () => {
      expect(findMinMaxValues([23, 45, 12, 67, 34])).toEqual({
        min: 12,
        max: 67,
      });
      expect(findMinMaxValues([10, 10, 10, 10, 10])).toEqual({
        min: 10,
        max: 10,
      });
      expect(findMinMaxValues([-5, 0, 5, 10, 15])).toEqual({ min: -5, max: 15 });
      expect(findMinMaxValues([100])).toEqual({ min: 100, max: 100 });
      expect(findMinMaxValues([3, -7, 0, 8, 15, 9])).toEqual({
        min: -7,
        max: 15,
      });
    });
    ...
    test("should handle negative and mixed arrays", async () => {
      ...
      expect(findMinMaxValues([-1, -2, -3, -4, -5])).toEqual({
        min: -5,
        max: -1,
      });
      expect(findMinMaxValues([-10, 10, -20, 20, -30])).toEqual({
        min: -30,
        max: 20,
      });
      expect(findMinMaxValues([1.5, 2.5, -1.5, 0, 3.5])).toEqual({
        min: -1.5,
        max: 3.5,
      });
